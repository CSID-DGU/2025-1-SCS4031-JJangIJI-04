...
public class CookieResolver {
    ...
    public void checkLoginRequired(HttpServletRequest request) {
        if (isTokenEmpty(request)) {
            throw new HankkiMoaException(ExceptionCode.AUTHENTICATION_TOKEN_EMPTY);
        }

        if (isRefreshTokenEmpty(request)) {
            throw new HankkiMoaException(ExceptionCode.AUTHENTICATION_REFRESH_TOKEN_EMPTY);
        }
    }
    ...
    public String extractAccessToken(HttpServletRequest request) {
        return extractToken(request, CookieProvider.ACCESS_TOKEN_COOKIE_NAME)
                .orElseThrow(() -> new HankkiMoaException(ExceptionCode.AUTHENTICATION_ACCESS_TOKEN_EMPTY));
    }

    public String extractRefreshToken(HttpServletRequest request) {
        return extractToken(request, CookieProvider.REFRESH_TOKEN_COOKIE_NAME)
                .orElseThrow(() -> new HankkiMoaException(ExceptionCode.AUTHENTICATION_REFRESH_TOKEN_EMPTY));
    }
